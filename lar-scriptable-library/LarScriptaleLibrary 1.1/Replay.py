...
from LarScriptaleLibrary.AdvancedFunction import *
...
def _mouse_move_worker(q):
    memorized_x, memorized_y = get_cursor_pos()
    while not stop_event.is_set():
        ...
        dx, dy = task
        try:
            target_x = memorized_x + dx
            target_y = memorized_y + dy
            mouse_move(target_x, target_y)
            memorized_x = target_x
            memorized_y = target_y
        except:
            pass
...
if __name__ == "__main__":
    start_replay("record_1_20260518_204426.txt")
    pass